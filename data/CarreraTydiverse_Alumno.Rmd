...
```{r}
cran %>% filter(package == "plotly", r_version >= "3.4.0")
```
...
```{r}
pack_sum <- by_package %>% summarise(count = n(), unique = n_distinct(ip_id), countries = n_distinct(country), avg_bytes = mean(size), med_bytes = median(size))
...
```{r}
top_counts <- cran %>% group_by(package) %>% summarise(count = n()) %>% arrange(desc(count)) %>% filter(row_number() <= quantile(count, probs = 0.99))
...
```{r}
top_counts_sorted <- cran %>% group_by(package) %>% summarise(count = n()) %>% arrange(desc(count)) %>% filter(row_number() <= quantile(count, probs = 0.99)) %>% arrange(desc(count))
...
```{r}
top_unique_sorted <- cran %>% group_by(package) %>% summarise(unique = n_distinct(ip_id)) %>% arrange(desc(unique)) %>% filter(row_number() <= quantile(unique, probs = 0.99)) %>% arrange(desc(unique))
...
```{r}
cran %>% select(ip_id, country, package, size) %>% mutate(size_mb = size / (2^20)) %>% filter(size_mb <= 0.5)
...
```{r}
library(ggplot2)
cran %>% group_by(country) %>% summarise(total_downloads = n()) %>% arrange(desc(total_downloads)) %>% mutate(cumulative_percentage = cumsum(total_downloads) / sum(total_downloads)) %>% filter(cumulative_percentage >= 0.9) %>% arrange(desc(total_downloads)) %>% ggplot(aes(x = country, y = total_downloads)) + geom_point() + labs(x = "Código de país", y = "Tráfico generado")
...
```{r}
students2_tidy <- students2 %>% pivot_longer(cols = starts_with("male"), names_to = c(".value", "class"), names_sep = "_")
...
```{r}
students3_tidy <- students3 %>% gather(key = "exam", value = "score", -name, na.rm = TRUE) %>% separate(col = exam, into = c("class", "exam_type"), sep = "_") %>% group_by(name, class, exam_type) %>% summarise(score = first(score)) %>% spread(key = exam_type, value = score)
...
```{r}
students3_tidy <- students3 %>% gather(key = "exam", value = "score", -name, na.rm = TRUE) %>% separate(col = exam, into = c("class", "exam_type"), sep = "_") %>% mutate(class = parse_number(class)) %>% group_by(name, class, exam_type) %>% summarise(score = first(score)) %>% spread(key = exam_type, value = score)
...
```{r}
student_info <- students4 %>% distinct(id, .keep_all = TRUE)

gradebook <- students4 %>% select(id, starts_with("exam"))
